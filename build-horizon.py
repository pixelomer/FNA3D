...
options = ['-DBUILD_SHARED_LIBS=OFF', '-DBUILD_VULKAN=OFF']
if 'FNA3D' == 'FNA3D':
...
run(['cmake', '--build', out, '--parallel', a.jobs, '--target', *['FNA3D', 'mojoshader']])
manifest = {'revision': subprocess.check_output(['git', '-C', ROOT, 'rev-parse', 'HEAD'], text=True).strip(),
            ...
            'archives': {name: digest(out / name) for name in ['libFNA3D.a', 'libmojoshader.a']}}
if 'FNA3D' == 'SDL': manifest['archive_sha256'] = digest(out / 'libSDL2.a')
